...
if (!isset($_SESSION['login']) || $_SESSION['login'] !== true) {
    logActivity("UNAUTHORIZED", "Unauthorized access attempt to Add RUPS MOM.");
    echo "<script type='text/javascript'>window.location='index'</script>";
    exit;
}
else { 
	$decoded = $rupsMOM->getData(); 
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['Title']) && isset($_POST['Judul']) && isset($_POST['Year']) && isset($_FILES['file2']) && isset($_POST['desc']) && isset($_POST['desc2']) && isset($_POST['addFH'])){ 
		...
		if (in_array($ext, $allowedExts) && in_array($mime, $allowedMime)) {
			if ($ukuran2 < 150 * 1024 * 1024) {
				$newFilename = uniqid('RUPS_MOM_', true) . '.pdf'; 
			 
				$add = $rupsMOM->addReport($judul, $title, $year, $desc, $deskripsi, $uniqueName, $date);
				if ($add) {
					move_uploaded_file($file_tmp2, "assets/pdf/rupsMOM/$newFilename");
					chmod("assets/pdf/rupsMOM/$newFilename", 0644);   
					logActivity("RUPS_MOM_ADD_SUCCESS", "RUPS Minutes of Meeting added: $newFilename by {$_SESSION['username']}");
					echo "<script type='text/javascript'>alert('RUPS Minutes of Meeting Added Success');</script>";
				}else{
					logActivity("RUPS_MOM_ADD_FAILED", "Duplicate or DB error on add.");
					echo "<script type='text/javascript'>alert('RUPS Minutes of Meeting Added Failed. PDF exsist');</script>";
				}
			}else{
				logActivity("FILE_TOO_LARGE", "Attempted to upload a too-large file for RUPS Minutes of Meeting ID $id.");
				echo "<script type='text/javascript'>alert('File Too Big');</script>";
			}
		}else{
			if (!in_array($ext, $allowedExts) || !in_array($mime, $allowedMime)) {
				logActivity("INVALID_FILE", "Attempted upload with invalid file type or MIME for RUPS Minutes of Meeting ID $id.");
			}
		...
		<div class="content-wrapper">
			...
			<section class="content-header">
				<div class="container-fluid">
					<div class="row mb-2">
						<div class="col-sm-6">
							<h1>GMS Minutes of Meeting</h1>
						</div>
						<div class="col-sm-6">
							<ol class="breadcrumb float-sm-right">
								<li class="breadcrumb-item"><a href="index">Home</a></li>
								<li class="breadcrumb-item active">GMS Minutes of Meeting</li>
							</ol>
						</div>
					</div>
				</div><!-- /.container-fluid -->
			...
			<section class="content-header">
				<div class="container-fluid">
					<div class="row mb-2">
						<div class="col-sm-6">
							<h1>Data GMS Minutes of Meeting</h1>
						</div>
					</div>
				</div><!-- /.container-fluid -->
			</section>
				...
				<div class="container-fluid">
					<div class="row">
						<div class="col-12">  
							<div class="card">
								<div class="card-header">
									<h3 class="card-title">Data GMS Minutes of Meeting</h3>
								</div>
								<!-- /.card-header -->
								<div class="card-body"> 
										...
										<tbody>
											...
											<?php }else{  for ($x = 0; $x < count($decoded); $x++) { ?>   
												<tr>
													...
													<td>
														<a href="editrupsMOM?id=<?php echo $decoded[$x]['ID_Laporan']; ?>" name="edit" class="btn btn-primary">Edit</a> 
														<a href="#myModal" class="btn btn-danger" data-href="deleterupsMOM?id=<?php echo $decoded[$x]['ID_Laporan']; ?>" data-toggle="modal" data-target="#myModal">Delete</a> 
													</td>
												</tr>
												<?php $no++; }
											}?>

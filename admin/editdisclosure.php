<?php 
$title = "Edit Disclosure Information | Paragon Karya Perkasa, Tbk"; 
...
if (!isset($_SESSION['login']) || $_SESSION['login'] !== true) {
    logActivity("UNAUTHORIZED", "Unauthorized access attempt to Edit Disclosure Information.");
    echo "<script type='text/javascript'>window.location='index'</script>";
    exit;
}else{
    $id = (int) $_GET['id'];
    $decoded = $disclosure->getDataByUid($id);  
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['Title']) && isset($_POST['Judul']) && isset($_POST['Year']) && isset($_FILES['file2']) && isset($_POST['desc']) && isset($_POST['desc2']) && isset($_POST['editRep'])){ 
        ...
        if(isset($_FILES["file2"]) && !empty($_FILES["file2"]["name"])){ 
            ...
            if (in_array($ext, $allowedExts) && in_array($mime, $allowedMime)) {  
                if($ukuran2 < 150*MB){  
                    ...
                    move_uploaded_file($tmpName, "assets/pdf/disclosure/$newFilename");
                ...
                if (!in_array($ext, $allowedExts) || !in_array($mime, $allowedMime)) {
                    logActivity("INVALID_FILE", "Attempted upload with invalid file type or MIME for Disclosure Information ID $id.");
                }
        ...
        if ($update) {
            logActivity("UPDATE", "Disclosure Information ID $id updated successfully" . ($file ? " with new file $newFilename." : " without file update."));
            echo "<script type='text/javascript'>alert('Disclosure Information Update Success');</script>";
        } else {
            logActivity("UPDATE_FAIL", "Failed to update Disclosure Information ID $id. Possibly duplicate PDF.");
            echo "<script type='text/javascript'>alert('Disclosure Information Update Failed. PDF exsist');</script>";
        } 
        ...
        <div class="content-wrapper">
            ...
            <section class="content-header">
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h1>Edit Disclosure Information</h1>
                        </div>
                        <div class="col-sm-6">
                            <ol class="breadcrumb float-sm-right">
                                <li class="breadcrumb-item"><a href="index">Home</a></li>
                                <li class="breadcrumb-item active">Edit Disclosure Information</li>
                            </ol>
                        </div>
                    </div>
                </div><!-- /.container-fluid -->

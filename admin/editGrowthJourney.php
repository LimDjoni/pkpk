<?php 
$title = "Edit Growth Journey | Paragon Karya Perkasa, Tbk"; 
...
if (!isset($_SESSION['login']) || $_SESSION['login'] !== true) {
    logActivity("UNAUTHORIZED", "Unauthorized access attempt to Edit Growth Journey.");
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['Title']) && isset($_POST['Judul']) && isset($_POST['Year']) && isset($_FILES['file2']) && isset($_POST['desc']) && isset($_POST['desc2']) && isset($_POST['editRep'])){ 
        ...
        if(isset($_FILES["file2"]) && !empty($_FILES["file2"]["name"])){ 
            ...
            if (in_array($ext, $allowedExts) && in_array($mime, $allowedMime)) {  
                if($ukuran2 < 150*MB){  
                    ...
                    move_uploaded_file($tmpName, "assets/img/growthjourney/$newFilename");
        ...
        if($file == true){ 
            $update = $growthjc->updateDataByUID($judul, $title, $year, $desc, $deskripsi, $newFilename, $date, $id);
        }else{
            $update = $growthjc->updateDataWithoutFileByUID($judul, $title, $year, $desc, $deskripsi, $date, $id);
        }
        
        if ($update) {
            logActivity("UPDATE", "Growth Journey ID $id updated successfully" . ($file ? " with new file $newFilename." : " without file update."));
            echo "<script type='text/javascript'>alert('Growth Journey Update Success');</script>";
        } else {
            logActivity("UPDATE_FAIL", "Failed to update Growth Journey ID $id. Possibly duplicate PDF.");
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="card-body col-md-6">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <textarea id="Remark" name="desc" class="form-control" rows="4" cols="50" placeholder="Enter Description"><?php echo $decoded['Des']; ?></textarea> 
                                            </div>
                                        </div>
                                        <div class="card-body col-md-6">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <textarea id="Remark" name="desc2" class="form-control" rows="4" cols="50" placeholder="Enter Description"><?php echo $decoded['Deskripsi']; ?></textarea> 
                                            </div>
                                        </div>

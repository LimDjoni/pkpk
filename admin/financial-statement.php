<?php 
$title = "Financial Statement | Perdana Karya Perkasa, Tbk"; 
...
if (!isset($_SESSION['login']) || $_SESSION['login'] !== true) {
    logActivity("UNAUTHORIZED", "Unauthorized access attempt to Add Financial Statement.");
    echo "<script type='text/javascript'>window.location='index'</script>";
    exit;
}
else {
	$decoded = $financialStatement->getData(); 
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['Title']) && isset($_POST['Judul']) && isset($_POST['Year']) && isset($_FILES['file2']) && isset($_POST['desc']) && isset($_POST['desc2']) && isset($_POST['addFH'])){ 
		...
		if (in_array($ext, $allowedExts) && in_array($mime, $allowedMime)) {
			if ($ukuran2 < 150 * 1024 * 1024) {
				...
				$add = $financialStatement->addReport($judul, $title, $year, $desc, $deskripsi, $uniqueName, $date);
				if ($add) {
					move_uploaded_file($file_tmp2, "assets/pdf/FinancialStatement/$newFilename");
					chmod("assets/pdf/FinancialStatement/$newFilename", 0644);  
					logActivity("FINANCIAL_STATEMENT_ADD_SUCCESS", "Financial Statement added: $newFilename by {$_SESSION['username']}");
					echo "<script type='text/javascript'>alert('Financial Statement Added Success');</script>";
				}else{
					logActivity("FINANCIAL_STATEMENT_ADD_FAILED", "Duplicate or DB error on add.");
					echo "<script type='text/javascript'>alert('Financial Statement Added Failed. PDF exsist');</script>";
				}
			}else{
		...
		<div class="content-wrapper">
			...
			<section class="content-header">
				<div class="container-fluid">
					<div class="row mb-2">
						<div class="col-sm-6">
							<h1>Data Financial Statement</h1>
						</div>
					</div>
				</div><!-- /.container-fluid -->
			</section>
				...
				<div class="container-fluid">
					<div class="row">
						<div class="col-12">  
							<div class="card">
								<div class="card-header">
									<h3 class="card-title">Data Financial Statement</h3>
								</div>
								<!-- /.card-header -->
								<div class="card-body"> 
										...
										<tbody>
											...
											<?php }else{  for ($x = 0; $x < count($decoded); $x++) { ?>   
												<tr>
													...
													<td>
														<a href="editFinancialStatement?id=<?php echo $decoded[$x]['ID_Laporan']; ?>" name="edit" class="btn btn-primary">Edit</a> 
														<a href="#myModal" class="btn btn-danger" data-href="deleteFinancialStatement?id=<?php echo $decoded[$x]['ID_Laporan']; ?>" data-toggle="modal" data-target="#myModal">Delete</a> 
													</td>
												</tr>
												<?php $no++; }
											}?>

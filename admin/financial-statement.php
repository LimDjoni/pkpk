...
if($_SESSION['login'] == true) {
	...
	if (isset($_POST['Title']) && isset($_POST['Judul']) && isset($_POST['Year']) && isset($_FILES['file2']) && isset($_POST['desc']) && isset($_POST['desc2']) && isset($_POST['addFH'])){ 
		...
		if(in_array($ekstensi2, $ekstensi_diperbolehkan2) === true){
			if($ukuran2 < 150*MB){   
				$add = $financialStatement->addReport($judul, $title, $year, urlencode($desc), urlencode($deskripsi), $nama2, $date);
				if($add){
					// move_uploaded_file($file_tmp, 'assets/img/FinancialStatement/'.$nama);
					move_uploaded_file($file_tmp2, 'assets/pdf/FinancialStatement/'.$nama2);
					// chmod('assets/img/FinancialStatement/'.$nama, 0664);

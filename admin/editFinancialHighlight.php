...
if($_SESSION['login'] == true) {
    ...
    if (isset($_POST['Title']) && isset($_POST['Judul']) && isset($_POST['Year']) && isset($_FILES['file2']) && isset($_POST['desc']) && isset($_POST['desc2']) && isset($_POST['editRep'])){ 
        ...
        if(in_array($ekstensi2, $ekstensi_diperbolehkan2) === true){
                if($ukuran2 < 150*MB){  
                    $update = $financialHighlight->updateDataByUID($judul, $title, $year, urlencode($desc), urlencode($deskripsi), $nama2, $date, $id);
                    if($update){
            ...
            $update = $financialHighlight->updateDataWithoutFileByUID($judul, $title, $year, urlencode($desc), urlencode($deskripsi), $date, $id);
            if($update){
                echo "<script type='text/javascript'>alert('Financial Highlight Update Success');</script>";
            }else{
                echo "<script type='text/javascript'>alert('Financial Highlight Update Failed. PDF exsist');</script>";

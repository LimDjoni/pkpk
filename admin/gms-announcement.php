...
if($_SESSION['login'] == true) {
	...
	if (isset($_POST['Title']) && isset($_POST['Judul']) && isset($_POST['Year']) && isset($_FILES['file2']) && isset($_POST['desc']) && isset($_POST['desc2']) && isset($_POST['addFH'])){ 
		...
		if(in_array($ekstensi2, $ekstensi_diperbolehkan2) === true){
			if($ukuran2 < 150*MB){   
				$add = $rupsAnnounce->addReport($judul, $title, $year, urlencode($desc), urlencode($deskripsi), $nama2, $date);
				if($add){
					// move_uploaded_file($file_tmp, 'assets/img/rupsAnnounce/'.$nama);
					move_uploaded_file($file_tmp2, 'assets/pdf/rupsAnnounce/'.$nama2);
					// chmod('assets/img/rupsAnnounce/'.$nama, 0664);
					chmod('assets/pdf/rupsAnnounce/'.$nama2, 0777);
					echo "<script type='text/javascript'>alert('GMS Announcement Added Success');</script>";
				}else{
					echo "<script type='text/javascript'>alert('GMS Announcement Added Failed. PDF exsist');</script>";
				}
			}else{

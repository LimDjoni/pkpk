<?php 
$title = "GMS Announcement | Perdana Karya Perkasa, Tbk"; 
...
if (!isset($_SESSION['login']) || $_SESSION['login'] !== true) {
    logActivity("UNAUTHORIZED", "Unauthorized access attempt to Add RUPS Announcement.");
    echo "<script type='text/javascript'>window.location='index'</script>";
    exit;
}
else {
	$decoded = $rupsAnnounce->getData(); 
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['Title']) && isset($_POST['Judul']) && isset($_POST['Year']) && isset($_FILES['file2']) && isset($_POST['desc']) && isset($_POST['desc2']) && isset($_POST['addFH'])){ 
		...
		if (in_array($ext, $allowedExts) && in_array($mime, $allowedMime)) {
			if ($ukuran2 < 150 * 1024 * 1024) {
				...
				$add = $rupsAnnounce->addReport($judul, $title, $year, $desc, $deskripsi, $uniqueName, $date);
				if ($add) {
					move_uploaded_file($file_tmp2, "assets/pdf/rupsAnnounce/$newFilename");
					chmod("assets/pdf/rupsAnnounce/$newFilename", 0644);  
					logActivity("RUPS_ANNOUNCE_ADD_SUCCESS", "RUPS Announce added: $newFilename by {$_SESSION['username']}"); 
					echo "<script type='text/javascript'>alert('RUPS Announce Added Success');</script>";
				}else{
					logActivity("RUPS_ANNOUNCE_ADD_FAILED", "Duplicate or DB error on add.");
					echo "<script type='text/javascript'>alert('RUPS Announce Added Failed. PDF exsist');</script>";
				}
			}else{
					...
					<div class="row mb-2">
						...
						<div class="col-sm-6">
							<ol class="breadcrumb float-sm-right">
								...
								<li class="breadcrumb-item active">GMS Announcement</li>
							</ol>
						</div>
					</div>
				</div><!-- /.container-fluid -->
				...
				<div class="container-fluid">
					<div class="row">
						<div class="col-12">  
							<div class="card">
								<div class="card-header">
									<h3 class="card-title">Data GMS Announcement</h3>
								</div>
								<!-- /.card-header -->
								<div class="card-body"> 
										...
										<tbody>
											...
											<?php }else{  for ($x = 0; $x < count($decoded); $x++) { ?>   
												<tr>
													...
													<td>
														<a href="editrupsAnnounce?id=<?php echo $decoded[$x]['ID_Laporan']; ?>" name="edit" class="btn btn-primary">Edit</a> 
														<a href="#myModal" class="btn btn-danger" data-href="deleterupsAnnounce?id=<?php echo $decoded[$x]['ID_Laporan']; ?>" data-toggle="modal" data-target="#myModal">Delete</a> 
													</td>
												</tr>
												<?php $no++; }
											}?>

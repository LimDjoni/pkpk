<?php 
$title = "Annual Report | Perdana Karya Perkasa, Tbk"; 
...
if (!isset($_SESSION['login']) || $_SESSION['login'] !== true) {
    logActivity("UNAUTHORIZED", "Unauthorized access attempt to Add Annual Report.");
    echo "<script type='text/javascript'>window.location='index'</script>";
    exit;
}
else {
	$decoded = $laporan->getData();
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['Title']) && isset($_POST['Judul']) && isset($_POST['Year']) && isset($_FILES['file2']) && isset($_POST['desc']) && isset($_POST['desc2']) && isset($_POST['addRep'])){ 
		...
		if (in_array($ext, $allowedExts) && in_array($mime, $allowedMime)) {
			if ($ukuran2 < 150 * 1024 * 1024) {
				...
				$add = $laporan->addReport($judul, $title, $year, $desc, $deskripsi, $newFilename, $date);
				if ($add) {
					move_uploaded_file($file_tmp2, "assets/pdf/Upload/$newFilename"); 
					chmod("assets/pdf/Upload/$newFilename", 0644); 
					logActivity("ANNUAL_REPORT_ADD_SUCCESS", "Annual Report added: $newFilename by {$_SESSION['username']}");
					echo "<script type='text/javascript'>alert('Annual Report Added Success');</script>";
				}else{
					logActivity("ANNUAL_REPORT_ADD_FAILED", "Duplicate or DB error on add.");
					echo "<script type='text/javascript'>alert('Annual Report Added Failed. PDF exsist');</script>";
				}
			}else{
							...
							<div class="card card-primary">
								...
								<form action="" method="POST" enctype="multipart/form-data">
									...
									<div class="card-footer">
										<button type="submit" name="addRep" class="btn btn-primary">Submit</button>
									</div>
								</form>
							</div>
							<!-- /.card -->
				...
				<div class="container-fluid">
					<div class="row">
						<div class="col-12">  
							<div class="card">
								<div class="card-header">
									<h3 class="card-title">Data Annual Report</h3>
								</div>
								<!-- /.card-header -->
								<div class="card-body"> 
										...
										<tbody>
											...
											<?php }else{  for ($x = 0; $x < count($decoded); $x++) { ?>   
												<tr>
													...
													<td>
														<a href="editAnnualReport?id=<?php echo $decoded[$x]['ID_Laporan']; ?>" name="edit" class="btn btn-primary">Edit</a> 
														<a href="#myModal" class="btn btn-danger" data-href="deleteAnnualReport?id=<?php echo $decoded[$x]['ID_Laporan']; ?>" data-toggle="modal" data-target="#myModal">Delete</a> 
													</td>
												</tr>
												<?php $no++; }
											}?>

...
if($_SESSION['login'] == true) {
    ...
    if (isset($_POST['Title']) && isset($_POST['Judul']) && isset($_POST['Year']) && isset($_FILES['file2']) && isset($_POST['desc']) && isset($_POST['desc2']) && isset($_POST['editRep'])){ 
        ...
        if(in_array($ekstensi2, $ekstensi_diperbolehkan2) === true){
                if($ukuran2 < 150*MB){  
                    $update = $rupsMOM->updateDataByUID($judul, $title, $year, urlencode($desc), urlencode($deskripsi), $nama2, $date, $id);
                    if($update){
                        move_uploaded_file($file_tmp2, 'assets/pdf/rupsMOM/'.$nama2); 
                        chmod('assets/pdf/rupsMOM/'.$nama2, 0777);
                        echo "<script type='text/javascript'>alert('GSM Minutes Of Meeting Update Success');</script>";
                    }else{
                        echo "<script type='text/javascript'>alert('GSM Minutes Of Meeting Update Failed. PDF exsist');</script>";
                    }
                }else{
            ...
            $update = $rupsMOM->updateDataWithoutFileByUID($judul, $title, $year, urlencode($desc), urlencode($deskripsi), $date, $id);
            if($update){
                echo "<script type='text/javascript'>alert('GSM Minutes Of Meeting Update Success');</script>";
            }else{
                echo "<script type='text/javascript'>alert('GSM Minutes Of Meeting Update Failed. PDF exsist');</script>";

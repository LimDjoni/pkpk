...
class financialStatementController{
	...
	public function getData(){
		$query = mysqli_query($this->conn,"SELECT * FROM financialstatement WHERE delete_date IS NULL ORDER BY created_date DESC");
	...
	public function getDatabyYear($year){
		$query = mysqli_query($this->conn,"SELECT * FROM financialstatement WHERE delete_date IS NULL AND tahun = '$year' ORDER BY created_date DESC");
		$jumdata= mysqli_num_rows($query);
		if($jumdata==0){
			$data="-";
		} else{
	...
	public function getDataLimit($startFrom, $perPage){
		$query = mysqli_query($this->conn,"SELECT * FROM financialstatement WHERE delete_date IS NULL ORDER BY created_date DESC LIMIT $startFrom, $perPage");
		$jumdata= mysqli_num_rows($query);
		if($jumdata==0){
			$data="-";
		} else{
	...
	public function getDataLimitbyYear($startFrom, $perPage, $year){
		$query = mysqli_query($this->conn,"SELECT * FROM financialstatement WHERE delete_date IS NULL AND tahun = '$year' ORDER BY created_date DESC LIMIT $startFrom, $perPage");
		$jumdata= mysqli_num_rows($query);
		if($jumdata==0){
			$data="-";
		} else{
	...
	public function getDataTahun(){
		$query = mysqli_query($this->conn,"SELECT DISTINCT Tahun FROM financialstatement ORDER BY created_date DESC");
		$jumdata= mysqli_num_rows($query);
		if($jumdata==0){
			$data="-";
		} else{
	...
	public function addReport($Judul, $Title, $Tahun, $Desc, $Deskripsi, $pdf, $createddate){
		...
		if ($count_row == 0){
			$query="INSERT INTO financialstatement(Judul, Title, Tahun, Des, Deskripsi, PDF, created_date) VALUES ('$Judul', '$Title', '$Tahun', '$Desc', '$Deskripsi', '$pdf', '$createddate')";
	...
	public function updateDataByUID($Judul, $Title, $Tahun, $Desc, $Deskripsi, $pdf, $updatedate, $uID){
		...
		$count_row = $result->num_rows;
		if ($count_row == 0){
			$query = "UPDATE financialstatement SET Judul = '$Judul', Title = '$Title', Tahun = '$Tahun', Des = '$Desc', Deskripsi = '$Deskripsi', PDF = '$pdf', update_date = '$updatedate' WHERE ID_Laporan = '$uID'";
			$result = mysqli_query($this->conn,$query) or die(mysqli_connect_errno()."Data cannot inserted");
			return $result;
		}
		else{
	...
	public function updateDataWithoutFileByUID($Judul, $Title, $Tahun, $Desc, $Deskripsi, $updatedate, $uID){
		$query = "UPDATE financialstatement SET Judul = '$Judul', Title = '$Title', Tahun = '$Tahun', Des = '$Desc', Deskripsi = '$Deskripsi', update_date = '$updatedate' WHERE ID_Laporan = '$uID'";
		$result = mysqli_query($this->conn,$query) or die(mysqli_connect_errno()."Data cannot inserted");
		return $result;
	}
